# sync/mqtt/opcua-sgm-005-to-mqtt.json — history and reasoning
...
or `node-red` — the target whose consumer carries `opcua-sgm-005-processdata`) connects DIRECTLY to the OPC UA server of sgm-005 (the endpoint of
`sites/werk1/sources/opcua/opcua-sgm-005-processdata.json`) and publishes every scope=hub attribute of that source over MQTT, one topic per
