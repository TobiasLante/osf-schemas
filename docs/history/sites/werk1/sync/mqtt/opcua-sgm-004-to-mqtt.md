# sync/mqtt/opcua-sgm-004-to-mqtt.json — history and reasoning
...
or `node-red` — the target whose consumer carries `opcua-sgm-004-processdata`) connects DIRECTLY to the OPC UA server of sgm-004 (the endpoint of
`sites/werk1/sources/opcua/opcua-sgm-004-processdata.json`) and publishes every scope=hub attribute of that source over MQTT, one topic per
