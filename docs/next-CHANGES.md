...
| i3x | `i3x/` generated: the profiles as i3X 1.0 Object Types, proven lossless | no, additive |
| mutate | every gating linter proven to catch its defect | no |

## Declared open, not decided here

- `validation/open-units.json`: 88 numeric attributes no source states a unit for, and
  `plastProgress`, which arrives as a 0..1 ratio while the profile says percent.
- `validation/relationship-types.json`: `PART_OF` is the reverse of four `HAS_*` types. i3X allows
  one `reverseOf` per type.
- Attribute names mirror source tags, so they are not renamed. Exceptions noted in
  `validation/naming-standard.md`.
