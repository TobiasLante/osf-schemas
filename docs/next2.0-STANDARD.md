# next2.0 — merged reference × next standard

`next2.0/` lifts the `next/` schemas onto the standard agreed with the reference side
(mirror of the reference `20260706`). Goal: **no design difference between the reference and next2.0.**
`next/` is left untouched; this is a parallel folder on the `next2.0` branch.

## Already aligned (next already carried these — the reference adopted them)
...
- **`scope`** stays a next-side architectural field (the reference does not populate it).
- **`standard`** value stays per side (`OSF` here, the reference value on the reference side).

## Reference ideas that are now THE standard (next-side population = TODO, not invented here)
...
These are content/population tasks on next's own domains (CNC/ERP/QMS/WMS), deliberately **not invented** here — mirrors how the reference still has to fill `semantics`/`kpiRefs`.
