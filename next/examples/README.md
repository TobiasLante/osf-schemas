...
A concrete, customer-specific instance of this hierarchy (Enterprise `ENT` → Site `SITE` →
Area `WL` → ProcessCell `PLANT` → 17 Units → 174 EquipmentModules → 34 ControlModules,
229 nodes total) exists as a **demo fixture**:

- File: **`next/sources/static/anchor-customer-demo.json`**
...
(`next/sources/static/anchor-customer-demo.json`) to seed the live KG (229 master-data
