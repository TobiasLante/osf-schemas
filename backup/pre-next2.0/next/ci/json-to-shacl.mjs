...
/** True for a predicate whose RHS is resolved CENTRALLY by the it-evaluator
 * (another attribute / $now via `valueAttr`, or the active recipe / ISA-95
 * ProductDefinition via `valueFrom`). These have NO static literal RHS, so they
 * cannot be expressed as an edge SHACL value-node component — the edge detector
 * (and thus the SHACL artifact, which mirrors the edge/literal path) ignores
 * them. They are evaluated only by the central it-evaluator. */
function isCentralOnlyPredicate(pred) {
  return !!pred && (pred.valueFrom !== undefined || pred.valueAttr !== undefined);
}

/** A constraint that any leg (when/require) resolves centrally is skipped from
 * the SHACL build artifact (edge/literal path only). */
function isCentralOnlyConstraint(c) {
  return isCentralOnlyPredicate(c.when) || isCentralOnlyPredicate(c.require);
}
...
export function renderProfile(profile, profiles) {
  ...
  // SHACL mirrors the edge/literal path; central-only constraints (valueAttr /
  // valueFrom — resolved by the it-evaluator) carry no static RHS and are
  // omitted from the artifact rather than serialized with a bogus literal.
  const renderable = [...constraints.keys()]
    .sort()
    .filter((id) => !isCentralOnlyConstraint(constraints.get(id)));
  ...
  const blocks = renderable.map((id) => renderConstraint(profile, id, constraints.get(id), attrs));
